...
- Fonte de dados: Bybit (sem bloqueio geo)
...
# ─── BYBIT DATA (sem restricao geo) ─────────────────────────────────────────
# Mapa de timeframes: nosso formato -> Bybit
TF_MAP = {
    "1m": "1", "5m": "5", "15m": "15",
    "1h": "60", "4h": "240"
}
...
def get_candles(tf, limit=120):
    """Busca candles do Bybit — sem bloqueio geografico."""
    bybit_tf = TF_MAP.get(tf, tf)
    ...
    if data.get("retCode") != 0:
        raise Exception(f"Bybit erro: {data.get('retMsg')}")
...
def analyze_image_with_claude(image_bytes):
    if not ANTHROPIC_KEY:
        raise Exception("ANTHROPIC_KEY nao configurada no Render!")
...
def handle_command(text, chat_id):
    ...
    if cmd in ("/help","/commands","/start","/tools","/skill"):
        send_telegram(
            ...
            "📡 Sinais automaticos ativos (Bybit data)",
            chat_id
        )
    elif cmd == "/status":
        try:
