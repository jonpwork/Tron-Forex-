...
def score_all(c_h4, c_h1, c_m15, c_m5, c_m1, h4_trend):
    ...
    # Determina direção de busca: h4_trend se definido, senão local
    local_trend = h4_trend
    if local_trend == "neutral":
        local_trend = get_trend(c_h1)
    if local_trend == "neutral":
        local_trend = get_trend(c_m15)

    # Testa ambas direções — pega a que encontrar pernada válida
    m1_e = None
    if local_trend != "neutral":
        m1_e = m1_entry(c_m1, local_trend)
    if not m1_e:
        # Força busca nas duas direções
        m1_e = m1_entry(c_m1, "up") or m1_entry(c_m1, "down")
    # Também tenta no M5 se M1 falhar
    if not m1_e:
        if local_trend != "neutral":
            m1_e = m1_entry(c_m5, local_trend)
        if not m1_e:
...
def handle_command(text, chat_id):
    ...
    if cmd in ("/help","/commands","/start","/tools","/skill"):
        send_telegram(
            ...
            "  🎯 M1: qualquer pernada + 50% correção\n"
            "  🏦 SMC: OB + FVG + Sweep + BOS\n"
            "  📐 Fibonacci OTE 61.8–79%\n"
            "  🎯 Score confluência (min 4/10)", chat_id)
    ...
    while True:
        try:
            ...
            print(f"[{datetime.utcnow().strftime('%H:%M')}] "
                  ...
                  f"M1-50up:{'✅' if m1_up else '❌'} "
                  f"M1-50dn:{'✅' if m1_down else '❌'} "
            ...
            if h4_trend == "neutral":
                t_h1  = get_trend(c_h1)
                t_m15 = get_trend(c_m15)
                active_trend = t_h1 if t_h1 != "neutral" else t_m15
            ...
            if active_trend != "neutral":
                ...
                # Sem tendência definida: varre M1/M5 em ambas direções
                check_elliott_signals(c_h4,c_h1,c_m15,c_m5,c_m1,"up",price)
                check_elliott_signals(c_h4,c_h1,c_m15,c_m5,c_m1,"down",price)

            # ── Sinal M1/M5 standalone: pernada + 50% ─────────────
            now_ts2 = time.time()
            dirs_to_test = ["up","down"] if active_trend=="neutral" else [active_trend, ("down" if active_trend=="up" else "up")]
            for tf_dir in dirs_to_test:
                for tf_candles, tf_name in [(c_m1,"M1"),(c_m5,"M5")]:
                    standalone = m1_entry(tf_candles, tf_dir)
                    if standalone:
                        sk = f"50pct_{tf_name}_{tf_dir}"
                        cooldown_m1 = 600  # 10 min entre sinais 50% do mesmo TF+dir
                        if now_ts2 - last_signal_time.get(sk, 0) >= cooldown_m1:
                            ...
                            break  # dispara só 1 TF por direção por ciclo
