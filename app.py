...
def fire_m1_50pct_signal(m1_e, direction, price, context_trend):
    ...
    Contexto de tendência local incluído na mensagem.
    """
    ep   = m1_e["entry"]
    sp   = m1_e["stop"]
    size = m1_e["wave_size"]
    ...
    if direction == "up":
        emoji  = "✅"; action = "COMPRA BUY"; direcao = "ALTA 📈"
        tp1 = m1_e["wave_end"]                   # retorna ao topo da pernada
        tp2 = m1_e["wave_end"] + size * 0.618    # extensão 161.8%
    else:
        emoji  = "🔴"; action = "VENDA SELL"; direcao = "BAIXA 📉"
        tp1 = m1_e["wave_end"]                   # retorna ao fundo da pernada
        tp2 = m1_e["wave_end"] - size * 0.618
    ...
    send_telegram(
        ...
        f"🔁 Correção: <b>{ret:.0%}</b> (zona 38–62%)\n"
        f"________________________\n"
        f"💰 Entrada: <b>${ep:,.2f}</b>\n"
        f"🛑 Stop:    <b>${sp:,.2f}</b>\n"
        f"🎯 TP1:     <b>${tp1:,.2f}</b>  R:R ≈ 1:{rr1}\n"
...
def main_loop():
    ...
    while True:
        try:
            ...
            # Usa tendência H4 se definida, senão detecta local por TF menor
            active_trend = h4_trend
            if h4_trend == "neutral":
                # Tenta tendência local no H1 ou M15
                t_h1  = get_trend(c_h1)
            ...
            # ── Sinal M1 standalone: pernada + 50% em qualquer direção ──
            # Dispara mesmo se H4 neutral — usa tendência local do M1/M5
            for tf_dir in (["up","down"] if active_trend == "neutral" else [active_trend]):
                m1_standalone = m1_entry(c_m1, tf_dir)
                if m1_standalone:
                    sk = f"M1_50pct_{tf_dir}"
                    now_ts2 = time.time()
                    cooldown_m1 = 900  # 15 min entre sinais M1 standalone
                    if now_ts2 - last_signal_time.get(sk, 0) >= cooldown_m1:
                        m5_dir = get_trend(c_m5)
                        fire_m1_50pct_signal(m1_standalone, tf_dir, price, m5_dir)
