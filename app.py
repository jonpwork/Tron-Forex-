...
# ══════════════════════════════════════════════════════════════
#   ENTRADA NO M1 — qualquer pernada + correção de 50%
#   (independente de onda impulsiva perfeita de Elliott)
# ══════════════════════════════════════════════════════════════
def m1_entry(c, direction):
    """
    Gatilho M1: detecta QUALQUER pernada recente e dispara quando
    o preço corrigir ~50% dessa pernada.
    Não exige onda impulsiva perfeita — basta:
      1) Uma pernada clara (swing alto→baixo ou baixo→alto)
      2) O preço atual estar próximo de 50% de correção dessa pernada
    """
    price = c[-1]["close"]

    # Tenta primeiro com pivots normais
    for n_try in [SWING_SMALL, 2, 1]:
        wave = last_wave(c, direction, n=n_try)
        if wave:
            start, end, _ = wave
            size = abs(end - start)
    ...
    if not wave:
        # Fallback: detecta pernada pelo máximo e mínimo das últimas N velas
        lookback = 20
    ...
    fifty = (start + end) / 2
    tol   = max(memory["zone_tol"], 0.12)   # tolerância ampliada para M1

    if abs(price - fifty) / size > tol:
        return None

    # Direção: preço deve estar entre o início e o fim da pernada
    if direction == "up":
        if price < min(start, end) or price > max(start, end):
            return None
    else:
        if price < min(start, end) or price > max(start, end):
            return None
    ...
    return {
        ...
        "gatilho":     f"50% da pernada ${min(start,end):,.0f}→${max(start,end):,.0f}"
    }
    ...
    while True:
        try:
            ...
            if h4_trend != "neutral":
                ...
                check_abc_signals(c_h4,c_h1,c_m15,c_m5,c_m1,h4_trend,price)
